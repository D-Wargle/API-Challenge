...
class WeatherService {
  ...
  private parseCurrentWeather(response: any): Weather {
    ...
    let { name: cityName } = response.city;
    let currentWeatherData = response.list[0];
    let {
  ...
  private buildForecastArray(_currentWeather: Weather, weatherData: any[]): Weather[] {
    ...
    for (let i = 1; i < weatherData.length; i++) {
      ...
      let { description, icon } = weather[0];
      let forecast = new Weather(this.cityName || '', date, icon, description, temp, humidity, windSpeed);
